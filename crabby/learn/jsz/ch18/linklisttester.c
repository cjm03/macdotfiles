...
typedef int ListData;                                                   // ListData     int synonym
typedef struct _Node ListNode;                                          // ListNode     _Node synonym
typedef enum { eFront = 0, eBack } eWhere;                              // eWhere       Enumeration Declaration
typedef enum {eLook = 0, eInsert, eDelete } eAction;                    // eAction      Enumeration Declaration
typedef struct _Node { ListNode* pNext; ListData* pData; } ListNode;    // ListNode     Structure Declaration
typedef struct { ListNode* pFirstNode; int nodeCount; } LinkedList;     // LinkedList   Structure Declaration
...
LinkedList*     /* returns an allocated LinkedList initialized to zeros */
CreateLinkedList()
{
...
bool            /* returns boolean TRUE if the list is empty */ 
IsEmpty(LinkedList* pList)
{
    return (pList->nodeCount == 0);
}

int             /* returns integer value of node count */ 
Size(LinkedList* pList)
{
    return pList->nodeCount;
}

void        
InsertNodeToFront(LinkedList* pList, ListNode* pNode)
{
...
void        
InsertNodeToBack(LinkedList* pList, ListNode* pNode)
{
    if (IsEmpty(pList)) {
        pList->pFirstNode = pNode;
    } else {
...
ListNode*
RemoveNodeFromFront(LinkedList* pList)
{
...
ListNode*
RemoveNodeFromBack(LinkedList* pList)
{
    if (IsEmpty(pList)) {
        return NULL;
    } else {
...
ListNode*
GetNode(LinkedList* pList, int pos)
{
    ListNode* pCurr = pList->pFirstNode;
    if (pCurr == NULL) {
...
void
PrintList(LinkedList* pList, void (*printData) (ListData* pData))
{
...
void
PrintNode(ListNode* pNode, void (*printData) (ListData* pData))
{
...
void
TestCreateNodeAndInsert(LinkedList* pLL, ListData data, eWhere where)
{
...
ListData
TestExamineNode(LinkedList* pLL, eWhere where)
{
    ListNode * pNode;
    switch (where) {
...
ListData
TestRemoveNodeAndFree(LinkedList* pLL, eWhere where)
{
    ListNode * pNode;
    switch (where) {
...
void
TestPrintOperation(LinkedList* pLL, eAction action, ListData data, eWhere where)
{
    switch (action) {
        case eLook:
